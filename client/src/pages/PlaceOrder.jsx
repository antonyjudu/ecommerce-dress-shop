...
function PlaceOrder() {
  ...
  const {navigate, userToken, cartItems, setCartItems, getCartAmount, delivery_fee, products} = useContext(ShopContext);
  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async (res) => {
        try {
          const { data } = await axios.post(backendUrl + '/api/order/verifyRazorpay', res, {headers: {usertoken: userToken}});
          if(data.success){
            navigate('/orders');
            setCartItems({});
          }
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      switch (method) {
        // COD
        case 'cod':
          const res = await axios.post(backendUrl + '/api/order/place', orderData, {headers: {usertoken: userToken}});
          if(res.data.success){
            setCartItems({});
            navigate('/orders');
          }else{
          ...
          break;
      case 'stripe':
        const resStripe = await axios.post(backendUrl + '/api/order/stripe', orderData, {headers: {usertoken: userToken}});
        ...
        break;
      case 'razorpay':
        const resRazorpay = await axios.post(backendUrl + '/api/order/razorpay', orderData, {headers: {usertoken: userToken}});
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            ...
            <div onClick={() => setMethod('razorpay')} className='flex items-center gap-3 border border-gray-300 p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-emerald-400' : ''}`}></p>
